...
function core_upgrade_preamble() {
	...
	if ( $updates && ( count( $updates ) > 1 || $updates[0]->response != 'latest' ) ) {
		...
		echo '<p>' . sprintf( __( '<a href="%s">Saber más about WordPress %s</a>.' ), esc_url( self_admin_url( 'about.php' ) ), $normalized_version ) . '</p>';
	}
	dismissed_updates();
}
...
<?php
	...
	show_message( '<span class="hide-if-js">' . sprintf( __( 'Welcome to WordPress %1$s. <a href="%2$s">Saber más</a>.' ), $result, esc_url( self_admin_url( 'about.php?updated' ) ) ) . '</span>' );
	?>
	</div>
	<script type="text/javascript">
